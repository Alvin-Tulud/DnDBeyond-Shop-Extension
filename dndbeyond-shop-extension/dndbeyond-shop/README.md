...
- `data/srd-items.json` is populated from a real `dnd5eapi.co` build-time pull via `js/fetch-srd.js`, not the hand-assembled placeholder this project started with — check the file's own `pulledAt` and `srdVersion` fields for what's currently in it, and re-run one of the commands below whenever you want to refresh it:
